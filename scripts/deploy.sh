...
deploy_to_instance() {
    ...
    local deployment_commands=(
        ...
        "sudo cp -r /opt/app /opt/app.backup.\$(date +%Y%m%d-%H%M%S) || true"
        "wget -O /tmp/deployment-artifact.tar.gz 'https://$ARTIFACTS_BUCKET.s3.$AWS_REGION.amazonaws.com/$artifact_path'"
        "sudo tar -xzf /tmp/deployment-artifact.tar.gz -C /opt/app --strip-components=1"
        "sudo chown -R app:app /opt/app"
        "sudo systemctl start app"
        "sudo systemctl enable app"
        "rm -f /tmp/deployment-artifact.tar.gz"
