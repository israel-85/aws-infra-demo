...
run_rollback_validation() {
    ...
    if [[ -f "app/tests/smoke/nginx-deployment.test.js" ]]; then
        ...
        # Get ALB DNS name for smoke tests
        local alb_dns
        alb_dns=$(cd "infrastructure/environments/$environment" && terraform output -raw alb_dns_name 2>/dev/null || echo "")
        
        if [[ -n "$alb_dns" ]]; then
            export ALB_DNS_NAME="$alb_dns"
            if (cd app && node tests/smoke/nginx-deployment.test.js); then
            ...
            warning "Could not determine ALB DNS name, skipping smoke tests"
        fi
    fi
